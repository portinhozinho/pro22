...
function setup() {
  ...
  ground = Bodies.rectangle(600,580,1200,10, options);
...
imageMode(CENTER)
  ...
  rectMode(CENTER)
  rect(ground.position.x, ground.position.y,0,0);
  pop();
  obj.display()
...
function showboats(){
  if(matris.length>0){
    if (matris[matris.length-1].body.position.x<900){var Barco= new barco(width+200, height-150,170,170)
    matris.push(Barco)}
for(var i=0; i<matris.length;i++){
  ...
  else{ var Barco= new barco(width+200, height-150,170,170) 
    matris.push(Barco)
  }
